...
function latexreport(jsonfile)
    sims = JSON.parsefile(jsonfile)
    ...
    for vol in all_keys
        ...
        for p in sort(collect(keys(parts)))
            ...
            body *= write_line_prim(sims[vol][p], pn, iso)
        end
        # write total if present
        if (haskey(sims[vol], "mc-mass-kg") ||
            haskey(sims[vol], "mc-volume-cm3") ||
            haskey(sims[vol], "mc-density-kgcm3"))
            body *= write_line_prim(sims[vol], "\\rclb\\textsc{total}", iso)
        end
    ...
    status = success(run(`pdflatex -output-directory=/tmp -interaction=nonstopmode /tmp/gerda-mage-sim-report-$user.tex`, wait=false))
    !status && return -1
