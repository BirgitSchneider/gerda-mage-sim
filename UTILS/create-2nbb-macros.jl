...
for f in [ 
          "../gedet/intrinsic/$(ARGS[1])/edep/log/raw-gedet-intrinsic-$(ARGS[1])-%VOL-%DET.tmac",
...
temLines = readlines(open("../gedet/intrinsic/$(ARGS[1])/edep/log/raw-gedet-intrinsic-$(ARGS[1])-%VOL-%DET.tmac"))
...
for (det, info) in gedMap
    ...
    for VOLTYPE ∈ ["AV", "DV"]
        ...
        writedlm(open("../gedet/intrinsic/$(ARGS[1])/edep/log/raw-gedet-intrinsic-$(ARGS[1])-$VOLTYPE-ch$CHANNEL.mac", "w"), macLines, '\n')
        println(" created ../gedet/intrinsic/$(ARGS[1])/edep/log/raw-gedet-intrinsic-$(ARGS[1])-$VOLTYPE-ch$CHANNEL.mac")
    end
end
