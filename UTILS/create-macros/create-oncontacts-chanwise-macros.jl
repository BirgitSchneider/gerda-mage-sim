...
if size(ARGS,1) <= 1
    println("USAGE: julia create-oncontacts-chanwise-macros.jl [nplus|pplus] [ISOTOPE]")
    exit()
end
...
for f in [ 
          "../../gedet/$(ARGS[1])_chanwise/$(ARGS[2])/edep/log/raw-gedet-$(ARGS[1])_chanwise-$(ARGS[2])-edep-%DET.tmac",
...
temLines = readlines(open("../../gedet/$(ARGS[1])_chanwise/$(ARGS[2])/edep/log/raw-gedet-$(ARGS[1])_chanwise-$(ARGS[2])-edep-%DET.tmac"))
...
for (det, info) in gedMap
   ...
   writedlm(open("../../gedet/$(ARGS[1])_chanwise/$(ARGS[2])/edep/log/raw-gedet-$(ARGS[1])_chanwise-$(ARGS[2])-edep-ch$CHANNEL.mac", "w"), macLines, '\n')
   println(" created ../../gedet/$(ARGS[1])_chanwise/$(ARGS[2])/edep/log/raw-gedet-$(ARGS[1])_chanwise-$(ARGS[2])-edep-ch$CHANNEL.mac")
end
