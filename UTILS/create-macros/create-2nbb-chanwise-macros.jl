#= create-2nbb-chanwise-macros.jl
 =
...
if size(ARGS,1) < 2
    println("USAGE: julia create-2nbb-chanwise-macros.jl [2nbb|2nbbLV] [000|001|002|...]")
    exit()
end
(ARGS[1] == "2nbb" || ARGS[1] == "2nbbLV") || error("Please specify the [2nbb|2nbbLV] option!")

isfile("./create-2nbb-chanwise-macros.jl") || error("Please run this script from where create-2nbb-chanwise-macros.jl is!")

for f in [ 
          "../../gedet/intrinsic_chanwise/$(ARGS[1])/edep/log/raw-gedet-intrinsic_chanwise-$(ARGS[1])-edep-%DET.tmac",
...
temLines = readlines(open("../../gedet/intrinsic_chanwise/$(ARGS[1])/edep/log/raw-gedet-intrinsic_chanwise-$(ARGS[1])-edep-%DET.tmac"))
...
for (det, info) in gedMap
   ...
   writedlm(open("../../gedet/intrinsic_chanwise/$(ARGS[1])/edep/log/raw-gedet-intrinsic_chanwise-$(ARGS[1])-edep-ch$CHANNEL-$(ARGS[2]).mac", "w"), macLines, '\n')
   println(" created ../../gedet/intrinsic_chanwise/$(ARGS[1])/edep/log/raw-gedet-intrinsic_chanwise-$(ARGS[1])-edep-ch$CHANNEL-$(ARGS[2]).mac")
end
