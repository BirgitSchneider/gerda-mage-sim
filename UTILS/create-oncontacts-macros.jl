#= create-oncontacts-macros.jl
 ...
 = or dead layer) starting for generic .tmac template files for simulations
...
for f in [ 
          "../gedet/$(ARGS[1])/$(ARGS[2])/edep/log/raw-gedet-$(ARGS[1])-$(ARGS[2])-edep-%DET.tmac",
...
temLines = readlines(open("../gedet/$(ARGS[1])/$(ARGS[2])/edep/log/raw-gedet-$(ARGS[1])-$(ARGS[2])-edep-%DET.tmac"))
...
for (det, info) in gedMap
   ...
   writedlm(open("../gedet/$(ARGS[1])/$(ARGS[2])/edep/log/raw-gedet-$(ARGS[1])-$(ARGS[2])-edep-ch$CHANNEL.mac", "w"), macLines, '\n')
   println(" created ../gedet/$(ARGS[1])/$(ARGS[2])/edep/log/raw-gedet-$(ARGS[1])-$(ARGS[2])-edep-ch$CHANNEL.mac")
end
